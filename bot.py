...
def _settings_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    for m in user_settings.WHISPER_MODELS:
        ...
        model_buttons.append(
            InlineKeyboardButton(text=label, callback_data=f"model:{m}")
        )
    ...
    for code, label in user_settings.LANGUAGES:
        ...
        btn = InlineKeyboardButton(
            text=f"{mark}{label}", callback_data=f"lang:{code}"
        )
        row.append(btn)
        if len(row) == 2:
...
async def cmd_start(message: Message) -> None:
    await message.answer(
        "👋 Привет! Отправь мне голосовое сообщение — я транскрибирую его "
...
async def handle_voice(message: Message) -> None:
    ...
    try:
        ...
        transcribed: str = await asyncio.to_thread(
            transcriber.transcribe, tmp_path, s.model
        )
        ...
        translated: str = await asyncio.to_thread(
            translator.translate, transcribed, s.language
        )
...
async def cmd_benchmark(message: Message) -> None:
    ...
    status = await message.answer(
        f"🧪 *Бенчмарк Whisper* | аудио: {duration} с\n\n⏳ Запускаю…"
    )
...
async def main() -> None:
    logger.info(
        "Starting bot (default model: %s). Listening…", config.WHISPER_MODEL
    )
